...
def printOutput(tilings):
    ...
    """
    tilings_filename = f"tilings_{WIDTH}x{HEIGHT}_{SHAPE_SUFFIX}_{len(tilings)}.txt"
...
def plotAllTilings(tilings):
    ...
    plt.tight_layout()
    plt.savefig(f"tilings_{WIDTH}x{HEIGHT}_{SHAPE_SUFFIX}_{len(tilings)}.png", format = "png", dpi=800) 
